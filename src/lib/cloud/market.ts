 ...
 *
 * All write operations require the user to be logged in (cloud/auth). The
...
export interface MarketTextbook {
  id: string;
  ownerId: string;
  owner?: Pick<CloudUser, 'id' | 'name' | 'avatarUrl'>;
  title: string;
...
export interface MarketComment {
  ...
  content: string;
  userId: string;
  user?: Pick<CloudUser, 'id' | 'name' | 'avatarUrl'>;
  textbookId: string;
 ...
 * Throws CloudError(402, 'PURCHASE_REQUIRED') when the user has not purchased it.
 */
export async function downloadTextbookBlob(id: string): Promise<Blob> {
  const { downloadUrl } = await getDownloadUrl(id);
  const res = await fetch(downloadUrl);
  if (!res.ok) throw new CloudError(res.status, `Failed to fetch textbook: ${res.status}`);
  return res.blob();
}
...
export async function publishTextbook(args: {
  ...
  const { uploadUrl, key } = await cloudJson<{ uploadUrl: string; key: string }>(
    '/v1/market/upload-init',
    { method: 'POST', body: { filename: args.filename, contentType: args.file.type || 'application/octet-stream' } },
  );
  ...
  if (!put.ok) throw new CloudError(put.status, `Upload failed: ${put.status}`);
