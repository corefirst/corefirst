/**
 * Thin HTTP client for the CoreFirst SaaS API (`corefirst-world`).
 *
 * Responsibilities:
...
function refreshAccessToken(): Promise<string | null> {
  ...
  pendingRefresh = (async () => {
    ...
    try {
      const res = await fetch(`${getSaasBaseUrl()}/v1/auth/refresh`, {
        method: 'POST',
...
export async function saasFetch(path: string, opts: RequestOpts = {}): Promise<Response> {
  const url = path.startsWith('http') ? path : `${getSaasBaseUrl()}${path}`;
...
export async function saasJson<T = any>(path: string, opts: RequestOpts = {}): Promise<T> {
  const res = await saasFetch(path, opts);
  let body: any = null;
  try { body = await res.json(); } catch { /* empty body */ }
  if (!res.ok) {
    const message = body?.error || `Request failed: ${res.status}`;
    throw new SaasError(res.status, message, body?.code);
  }
  return body as T;
}
