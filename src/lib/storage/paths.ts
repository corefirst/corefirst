...
/**
 * Slug formula: `{category}-{targetLang}-{ageGroup}-{topicSlug}`.
 *
 * We always include the topic now — without it, two different courses with
 * the same category/language/age silently overwrote each other (the previous
...
export function buildSlug(
  ...
  const baseParts = [category, targetLang, ageGroup]
    .map(asciiSlug)
    .filter(Boolean);
